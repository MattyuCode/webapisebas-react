...
export const PDFAsistencia = ({ data }) => {
  ...
  doc.text(
    "Reporte de Personas con Asistencia",
  ...
  const tableData = listaPersonas.map((row) => [
    row.idActivdadAsistencia.toString(),
    row.nombreActividad,
  ]);

  autoTable(doc, {
    head: [["ID Actividad", "Nombre Actividad"]],
    body: tableData,
    startY: 50,
    theme: "grid",
    headStyles: {
  ...
  doc.save(`Reporte_PersonasConAsistencia_${idPersona}.pdf`);
};
